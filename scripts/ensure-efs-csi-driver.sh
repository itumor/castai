#!/usr/bin/env bash
#
# ensure-efs-csi-driver.sh
#
# Idempotently installs the AWS EFS CSI driver on an EKS cluster as a
# managed addon. Required before CAST AI can provision EFS-backed PVCs
# for ReadWriteMany workloads on EKS.
#
# Usage:
#   ./scripts/ensure-efs-csi-driver.sh
#
# Environment:
...
IRSA_ROLE_NAME="${CLUSTER_NAME}-efs-csi-driver"
...
function verify_pods() {
  local pods
  pods="$(kubectl get pods -n "${SERVICE_ACCOUNT_NAMESPACE}" -l app.kubernetes.io/name=aws-efs-csi-driver -o jsonpath='{.items[*].metadata.name}' 2>/dev/null || true)"
  if [[ -z "${pods}" ]]; then
    fail "Addon is ACTIVE but no EFS CSI driver pods found in ${SERVICE_ACCOUNT_NAMESPACE} with label app.kubernetes.io/name=aws-efs-csi-driver."
  fi
  log "EFS CSI driver pods running: ${pods}"
}
...
function wait_for_addon_active() {
  ...
  while (( $(date +%s) < deadline )); do
    ...
    if [[ "${status}" == "ACTIVE" ]]; then
      log "EFS CSI driver addon is ACTIVE (attempt ${attempt})."
      return 0
    fi
    echo "  Status: ${status:-UNKNOWN} (attempt ${attempt}); sleeping ${POLL_INTERVAL_SECONDS}s..."
    sleep "${POLL_INTERVAL_SECONDS}"
  done
  fail "EFS CSI driver addon did not reach ACTIVE within ${TIMEOUT_SECONDS}s. Last status: ${status:-UNKNOWN}. Inspect with: aws eks describe-addon --cluster-name ${CLUSTER_NAME} --addon-name ${ADDON_NAME} --region ${AWS_REGION}"
}
...
if [[ "${ADDON_STATUS}" == "ACTIVE" ]]; then
  log "EFS CSI driver addon is already installed and ACTIVE."
  verify_pods
  exit 0
fi

if [[ -n "${ADDON_STATUS}" ]]; then
  log "EFS CSI driver addon exists with status '${ADDON_STATUS}'."
fi

# Step 2: ensure the IRSA role for the EFS CSI controller exists.
...
if [[ -n "${ROLE_ARN}" ]]; then
  ...
  log "Creating IRSA role '${IRSA_ROLE_NAME}' for the EFS CSI controller..."
...
if [[ -z "${ADDON_STATUS}" ]]; then
  log "Creating EFS CSI driver addon on cluster '${CLUSTER_NAME}'..."
  eksctl create addon \
    --cluster "${CLUSTER_NAME}" \
    --region "${AWS_REGION}" \
    --name "${ADDON_NAME}" \
  ...
  fail "EFS CSI driver addon is in ${ADDON_STATUS} state. Repair or delete it before rerunning: aws eks delete-addon --cluster-name ${CLUSTER_NAME} --addon-name ${ADDON_NAME} --region ${AWS_REGION}"
fi
